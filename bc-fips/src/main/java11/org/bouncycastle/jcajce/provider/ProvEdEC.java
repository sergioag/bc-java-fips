    ...
    extends AsymmetricAlgorithmProvider
{
    ...
    private static final PublicKeyConverter<AsymmetricXDHPublicKey> xPublicKeyConverter = new PublicKeyConverter<AsymmetricXDHPublicKey>()
    {
            ...
            else
            {
                ...
                try
                {
                    if (key instanceof XECPublicKey)
                    {
                        ...
                        if ("XDH".equalsIgnoreCase(jceAlg))
                        {
        ...
        AsymmetricXDHPublicKey getX25519PublicKey(BigInteger u)
            throws InvalidKeyException
        {
            return new AsymmetricXDHPublicKey(EdEC.X25519.getAlgorithm(),
                Arrays.reverseInPlace(BigIntegers.asUnsignedByteArray(X25519.POINT_SIZE, u)));
        }

        AsymmetricXDHPublicKey getX448PublicKey(BigInteger u)
            throws InvalidKeyException
        {
            return new AsymmetricXDHPublicKey(EdEC.X448.getAlgorithm(),
                Arrays.reverseInPlace(BigIntegers.asUnsignedByteArray(X448.POINT_SIZE, u)));
        }
    };
