...
class LM_OTS
{
    ...
    static byte[] lms_ots_generatePublicKey(LMOtsParameters parameter, byte[] I, int q, byte[] masterSecret)
    {
        ...
        //
        Digest publicContext = LmsDigestUtil.getDigest(parameter);
        ...
        SeedDerive derive = new SeedDerive(I, masterSecret, LmsDigestUtil.getDigest(parameter));
        derive.setQ(q);
        derive.setJ(0);
    ...
    public static byte[] lm_ots_validate_signature_calculate(LMSContext context)
    {
        ...
        Digest finalContext = LmsDigestUtil.getDigest(parameter);
