    ...
    implements SP80090DRBG
{
    ...
    private void init(Mac hMac, int securityStrength, EntropySource entropySource, byte[] personalizationString, byte[] nonce)
    {
        ...
        workingBuf._K = new byte[hMac.getMacSize()];
        workingBuf._V = new byte[hMac.getMacSize()];
        Arrays.fill(workingBuf._V, (byte)1);
    ...
    {
        _hMac.init(new KeyParameterImpl(workingBuf._K));

        _hMac.update(workingBuf._V, 0, workingBuf._V.length);
        _hMac.update(vValue);
        ...
        _hMac.doFinal(workingBuf._K, 0);

        _hMac.init(new KeyParameterImpl(workingBuf._K));
        _hMac.update(workingBuf._V, 0, workingBuf._V.length);

        _hMac.doFinal(workingBuf._V, 0);
    }
    ...
    public int generate(byte[] output, byte[] additionalInput, boolean predictionResistant)
    {
        ...
        int m = output.length / workingBuf._V.length;
        ...
        for (int i = 0; i < m; i++)
        {
            ...
            System.arraycopy(workingBuf._V, 0, rv, i * workingBuf._V.length, workingBuf._V.length);
        }

        if (m * workingBuf._V.length < rv.length)
        {
            _hMac.update(workingBuf._V, 0, workingBuf._V.length);
            _hMac.doFinal(workingBuf._V, 0);

            System.arraycopy(workingBuf._V, 0, rv, m * workingBuf._V.length, rv.length - (m * workingBuf._V.length));
        }
        ...
        throws Throwable
    {
        try
        {
            Arrays.fill(workingBuf._K, (byte)0);
            Arrays.fill(workingBuf._V, (byte)0);
        }
        finally
        {
            super.finalize();
        ...
        return new VariantInternalKatTest(algorithm)
        {
                ...
                throws Exception
            {
                ...
                try
                {
                    ...
                    workingBuf._K = Arrays.clone(internalValues[0]);
                    workingBuf._V = Arrays.clone(internalValues[1]);
