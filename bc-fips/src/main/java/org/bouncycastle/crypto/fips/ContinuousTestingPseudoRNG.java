         ...
         synchronized (this)
         {
             ...
             if (block.length != output.length)
             {
                 if (block.length < output.length)
                 {
                     ...
                     // FSM_TRANS:5.4, "CONDITIONAL TEST", "CONTINUOUS DRBG TEST", "Invoke Continuous DRBG test"
                     if (rv < 0)
                     {
                         // FSM_TRANS:5.6, "CONTINUOUS DRBG TEST", "SOFT ERROR", "Continuous DRBG test failed"
                         FipsStatus.moveToErrorStatus("DRBG unable to initialise");
                     }
                     // FSM_TRANS:5.5, "CONTINUOUS DRBG TEST", "CONDITIONAL TEST", "Continuous DRBG test successful"
                 }
