...
private static final int[] Tinv0 =
    ...
    */
    ...
    private byte[]      s;
    private WorkingBuffer workingBuf;
        ...
        CipherParameters  params)
    {
        if (params instanceof KeyParameter)
        {
            ...
            this.workingBuf = new WorkingBuffer(generateWorkingKey(((KeyParameter)params).getKey(), forEncryption));
            return;
        }
        ...
        int outOff)
    {
        ...
        if (forEncryption)
        {
            encryptBlock(in, inOff, workingBuf.WorkingKey, out, outOff);
        }
        else
        {
            decryptBlock(in, inOff, workingBuf.WorkingKey, out, outOff);
        }
    ...
    private void encryptBlock(byte[] input, int inOff, int[][] KW, byte[] out, int outOff)
    {
        ...
        {
            kw = KW[r++];
            r0 = T0[t0&255] ^ shift(T0[(t1>>>8)&255], 24) ^ shift(T0[(t2>>>16)&255], 16) ^ shift(T0[(r3>>>24)], 8) ^ kw[0];
            r1 = T0[t1&255] ^ shift(T0[(t2>>>8)&255], 24) ^ shift(T0[(r3>>>16)&255], 16) ^ shift(T0[(t0>>>24)], 8) ^ kw[1];
            r2 = T0[t2&255] ^ shift(T0[(r3>>>8)&255], 24) ^ shift(T0[(t0>>>16)&255], 16) ^ shift(T0[(t1>>>24)], 8) ^ kw[2];
            r3 = T0[r3&255] ^ shift(T0[(t0>>>8)&255], 24) ^ shift(T0[(t1>>>16)&255], 16) ^ shift(T0[(t2>>>24)], 8) ^ kw[3];

            kw = KW[r++];
            t0 = T0[r0&255] ^ shift(T0[(r1>>>8)&255], 24) ^ shift(T0[(r2>>>16)&255], 16) ^ shift(T0[(r3>>>24)], 8) ^ kw[0];
            t1 = T0[r1&255] ^ shift(T0[(r2>>>8)&255], 24) ^ shift(T0[(r3>>>16)&255], 16) ^ shift(T0[(r0>>>24)], 8) ^ kw[1];
            t2 = T0[r2&255] ^ shift(T0[(r3>>>8)&255], 24) ^ shift(T0[(r0>>>16)&255], 16) ^ shift(T0[(r1>>>24)], 8) ^ kw[2];
            r3 = T0[r3&255] ^ shift(T0[(r0>>>8)&255], 24) ^ shift(T0[(r1>>>16)&255], 16) ^ shift(T0[(r2>>>24)], 8) ^ kw[3];
        }
        ...
        int k = kw[0];
        out[outOff++] = (byte)(S[r0&255] ^ k);
        out[outOff++] = (byte)(S[(r1>>>8)&255] ^ (k >>> 8));
        out[outOff++] = (byte)(s[(r2>>>16)&255] ^ (k >>> 16));
        out[outOff++] = (byte)(s[(r3>>>24)] ^ (k >>> 24));

        k = kw[1];
        out[outOff++] = (byte)(s[r1&255] ^ k);
        out[outOff++] = (byte)(S[(r2>>>8)&255] ^ (k >>> 8));
        out[outOff++] = (byte)(S[(r3>>>16)&255] ^ (k >>> 16));
        out[outOff++] = (byte)(s[(r0>>>24)] ^ (k >>> 24));

        k = kw[2];
        out[outOff++] = (byte)(s[r2&255] ^ k);
        out[outOff++] = (byte)(S[(r3>>>8)&255] ^ (k >>> 8));
        out[outOff++] = (byte)(S[(r0>>>16)&255] ^ (k >>> 16));
        out[outOff++] = (byte)(S[(r1>>>24)] ^ (k >>> 24));

        k = kw[3];
        out[outOff++] = (byte)(s[r3&255] ^ k);
        out[outOff++] = (byte)(s[(r0>>>8)&255] ^ (k >>> 8));
        out[outOff++] = (byte)(s[(r1>>>16)&255] ^ (k >>> 16));
        out[outOff] = (byte)(S[(r2>>>24)] ^ (k >>> 24));
    }
    ...
    private void decryptBlock(byte[] input, int inOff, int[][] KW, byte[] out, int outOff)
    {
        ...
        {
            kw = KW[r--];
            r0 = Tinv0[t0&255] ^ shift(Tinv0[(r3>>>8)&255], 24) ^ shift(Tinv0[(t2>>>16)&255], 16) ^ shift(Tinv0[(t1>>>24)], 8) ^ kw[0];
            r1 = Tinv0[t1&255] ^ shift(Tinv0[(t0>>>8)&255], 24) ^ shift(Tinv0[(r3>>>16)&255], 16) ^ shift(Tinv0[(t2>>>24)], 8) ^ kw[1];
            r2 = Tinv0[t2&255] ^ shift(Tinv0[(t1>>>8)&255], 24) ^ shift(Tinv0[(t0>>>16)&255], 16) ^ shift(Tinv0[(r3>>>24)], 8) ^ kw[2];
            r3 = Tinv0[r3&255] ^ shift(Tinv0[(t2>>>8)&255], 24) ^ shift(Tinv0[(t1>>>16)&255], 16) ^ shift(Tinv0[(t0>>>24)], 8) ^ kw[3];

            kw = KW[r--];
            t0 = Tinv0[r0&255] ^ shift(Tinv0[(r3>>>8)&255], 24) ^ shift(Tinv0[(r2>>>16)&255], 16) ^ shift(Tinv0[(r1>>>24)], 8) ^ kw[0];
            t1 = Tinv0[r1&255] ^ shift(Tinv0[(r0>>>8)&255], 24) ^ shift(Tinv0[(r3>>>16)&255], 16) ^ shift(Tinv0[(r2>>>24)], 8) ^ kw[1];
            t2 = Tinv0[r2&255] ^ shift(Tinv0[(r1>>>8)&255], 24) ^ shift(Tinv0[(r0>>>16)&255], 16) ^ shift(Tinv0[(r3>>>24)], 8) ^ kw[2];
            r3 = Tinv0[r3&255] ^ shift(Tinv0[(r2>>>8)&255], 24) ^ shift(Tinv0[(r1>>>16)&255], 16) ^ shift(Tinv0[(r0>>>24)], 8) ^ kw[3];
        }
        ...
        int k = kw[0];
        out[outOff++] = (byte)(Si[r0&255] ^ k);
        out[outOff++] = (byte)(s[(r3>>>8)&255] ^ (k >>> 8));
        out[outOff++] = (byte)(s[(r2>>>16)&255] ^ (k >>> 16));
        out[outOff++] = (byte)(Si[(r1>>>24)] ^ (k >>> 24));

        k = kw[1];
        out[outOff++] = (byte)(s[r1&255] ^ k);
        out[outOff++] = (byte)(s[(r0>>>8)&255] ^ (k >>> 8));
        out[outOff++] = (byte)(Si[(r3>>>16)&255] ^ (k >>> 16));
        out[outOff++] = (byte)(s[(r2>>>24)] ^ (k >>> 24));

        k = kw[2];
        out[outOff++] = (byte)(s[r2&255] ^ k);
        out[outOff++] = (byte)(Si[(r1>>>8)&255] ^ (k >>> 8));
        out[outOff++] = (byte)(Si[(r0>>>16)&255] ^ (k >>> 16));
        out[outOff++] = (byte)(s[(r3>>>24)] ^ (k >>> 24));

        k = kw[3];
        out[outOff++] = (byte)(Si[r3&255] ^ k);
        out[outOff++] = (byte)(s[(r2>>>8)&255] ^ (k >>> 8));
        out[outOff++] = (byte)(s[(r1>>>16)&255] ^ (k >>> 16));
        out[outOff] = (byte)(s[(r0>>>24)] ^ (k >>> 24));
    }
