...
class Utils
{
    ...
    static void checkDigestAlgorithm(Logger log, Algorithm algorithm, String override)
    {
        if (CryptoServicesRegistrar.isInApprovedOnlyMode())
        {
            if (FipsSHS.Algorithm.SHA1.equals(algorithm))
            {
                if (!Properties.isOverrideSet(override))
                {
                    throw new FipsUnapprovedOperationError("SHA-1 is not approved for signature generation");
                }
                else
                {
                    log.warning("[" + log.getName() + "] " + "SHA-1 signature creation detected: no longer an approved operation but override set");
                }
            }
        }
    }
