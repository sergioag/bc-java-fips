...
{
    private static final Logger LOG = Logger.getLogger(FipsRSA.class.getName());
    ...
    static
    {
        ...
        // FSM_STATE:5.RSA.0,"RSA SIGN VERIFY KAT", "The module is performing RSA sign and verify KAT self-test"
        // FSM_TRANS:5.RSA.0.0,"CONDITIONAL TEST","RSA SIGN VERIFY KAT", "Invoke RSA Sign/Verify KAT self-test"
        rsaSignTest(provider);
        // FSM_TRANS:5.RSA.0.1,"RSA SIGN VERIFY KAT","CONDITIONAL TEST", "RSA Sign/Verify KAT self-test successful completion"
        // FSM_TRANS:5.RSA.0.2,"RSA SIGN VERIFY KAT","SOFT ERROR", "RSA Sign/Verify KAT self-test failed"

        // FSM_STATE:5.RSA.1,"KEY AGREEMENT USING RSA VERIFY KAT", "The module is performing RSA Key Agreement verify KAT self-test"
        // FSM_TRANS:5.RSA.1.0,"CONDITIONAL TEST","KEY AGREEMENT USING RSA VERIFY KAT", "Invoke Key Agreement Using RSA, Specific SP 800-56B KAT self-test"
        rsaKasTest(provider);
        // FSM_TRANS:5.RSA.1.1, "KEY AGREEMENT USING RSA VERIFY KAT", "CONDITIONAL TEST", "Key Agreement Using RSA, Specific SP 800-56B KAT self-test successful completion"
        // FSM_TRANS:5.RSA.1.2, "KEY AGREEMENT USING RSA VERIFY KAT", "SOFT ERROR", "Key Agreement Using RSA, Specific SP 800-56B KAT self-test failed"

        // FSM_STATE:5.RSA.2, "KEY TRANSPORT USING RSA VERIFY KAT", "The module is performing RSA Key Transport verify KAT self-test"
        // FSM_TRANS:5.RSA.2.0,"CONDITIONAL TEST","KEY TRANSPORT USING RSA VERIFY KAT", "Invoke Key Transport Using RSA, Specific SP 800-56B KAT self-test"
        rsaKeyTransportTest(provider);
        // FSM_TRANS:5.RSA.2.1,"KEY TRANSPORT USING RSA VERIFY KAT", "CONDITIONAL TEST", "Key Transport Using RSA, Specific SP 800-56B KAT self-test successful completion"
        // FSM_TRANS:5.RSA.2.2,"KEY TRANSPORT USING RSA VERIFY KAT", "SOFT ERROR", "Key Transport Using RSA, Specific SP 800-56B KAT failed"
        ...
        extends FipsAsymmetricKeyPairGenerator<KeyGenParameters, AsymmetricRSAPublicKey, AsymmetricRSAPrivateKey>
    {
        ...
        public AsymmetricKeyPair<AsymmetricRSAPublicKey, AsymmetricRSAPrivateKey> generateKeyPair()
        {
            ...
            // FSM_STATE:5.RSA.3, "RSA PAIRWISE CONSISTENCY TEST", "The module is performing RSA Pairwise Consistency self-test"
            // FSM_TRANS:5.RSA.3.0,"CONDITIONAL TEST", "RSA PAIRWISE CONSISTENCY TEST", "Invoke RSA Pairwise Consistency test"
            validateKeyPair(kp);
            // FSM_TRANS:5.RSA.3.1,"RSA PAIRWISE CONSISTENCY TEST", "CONDITIONAL TEST", "RSA Pairwise Consistency test successful"
            // FSM_TRANS:5.RSA.3.2,"RSA PAIRWISE CONSISTENCY TEST", "SOFT ERROR", "RSA Pairwise Consistency test failed"
        ...
        extends FipsSignatureOperatorFactory<T>
    {
        ...
        public FipsOutputSignerUsingSecureRandom<T> createSigner(AsymmetricPrivateKey key, final T parameters)
        {
            ...
            // FSM_STATE:5.RSA.4,"RSA KEY USAGE CHECK", "The module verifies recent usage of an RSA key is consistent with requested usage"
            // FSM_TRANS:5.RSA.4.0,"CONDITIONAL TEST", "RSA KEY USAGE CHECK", "Invoke RSA key usage check"
            if (!k.canBeUsed(AsymmetricRSAKey.Usage.SIGN_OR_VERIFY))
            {
                // FSM_TRANS:5.RSA.4.2,"RSA KEY USAGE CHECK", "USER COMMAND REJECTED", "RSA key usage check failed"
                ...
                if (random != null)
                {
                    if (CryptoServicesRegistrar.isInApprovedOnlyMode())
                    {
                        if (parameters.getAlgorithm().equals(ALGORITHM_PKCS1v1_5))
                        {
                            ...
                            {
                                LOG.warning("RSA PKCS1.5 encryption detected: no longer an approved operation but override set");
                            }
                        }
                    }
                ...
                throws Exception
            {
                ...
                GeneratorImpl genImp = new GeneratorImpl(new RsaKeyParameters(false, katM, katE), new KTSParameters(ALGORITHM_SVE),
                    new FixedSecureRandom(
                        new FixedSecureRandom.Data(2048, zVal),
