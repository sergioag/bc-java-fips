...
public final class RSA
{
        ...
        extends SignatureParameters
    {
        ...
        private final DigestAlgorithm mgfDigest;
        private final int trailer;
        private final byte[] salt;

        PSSSignatureParameters()
        {
            this(FipsSHS.Algorithm.SHA1, FipsSHS.Algorithm.SHA1, 20, null, (byte)0xBC);
        }

        private PSSSignatureParameters(DigestAlgorithm digestAlgorithm, DigestAlgorithm mgfDigest, int saltLength, byte[] salt, int trailer)
        {
         ...
         * @return a new parameter set.
         */
        public PSSSignatureParameters withDigestAlgorithm(DigestAlgorithm digestAlgorithm)
        {
            return new PSSSignatureParameters(digestAlgorithm, digestAlgorithm, Register.createDigest(digestAlgorithm).getDigestSize(), null, trailer);
        }
         ...
         * @return a new parameter set.
         */
        public PSSSignatureParameters withMGFDigest(DigestAlgorithm mgfDigest)
        {
        ...
        extends GuardedSignatureOperatorFactory<T>
    {
        ...
        @Override
        public OutputValidator<T> doCreateValidator(AsymmetricPublicKey key, final SignatureParameters parameters, final byte[] signature)
        {
            AsymmetricRSAPublicKey k = (AsymmetricRSAPublicKey)key;
        ...
        extends GuardedSignatureWithMessageRecoveryOperatorFactory<T>
    {
        ...
        @Override
        protected OutputValidatorWithMessageRecovery<T> doCreateValidator(AsymmetricPublicKey key, final SignatureParameters parameters, final byte[] signature)
        {
            AsymmetricRSAPublicKey k = (AsymmetricRSAPublicKey)key;
            ...
            return new OutputValidatorWithMessageRecovery<T>()
            {
                public T getParameters()
                {
                    return (T)parameters;
                ...
                public void updateWithRecoveredMessage()
                    throws InvalidSignatureException
                {
                    try
                    {
