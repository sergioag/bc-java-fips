...
public final class ARIA
{
     ...
     */
    public static final GeneralAlgorithm ALGORITHM = new GeneralAlgorithm("ARIA");
    ...
    private static ValidatedSymmetricKey validateKey(SymmetricKey key, Algorithm genAlgorithm)
    {
        ...
        {
            throw new IllegalKeyException("ARIA key must be of length 128, 192, or 256");
        }
        ...
        implements org.bouncycastle.crypto.internal.EngineProvider<BlockCipher>
    {
        ...
        {
            return SelfTestExecutor.validate(ALGORITHM, new ARIAEngine(), new VariantKatTest<ARIAEngine>()
            {
                public void evaluate(ARIAEngine engine)
                {
                    byte[] input = Hex.decode("00112233445566778899aabbccddeeff");
                    byte[] output = Hex.decode("d718fbd6ab644c739da95f3be6451778");
                    byte[] tmp = new byte[input.length];

                    KeyParameter key = new KeyParameterImpl(Hex.decode("000102030405060708090a0b0c0d0e0f"));
