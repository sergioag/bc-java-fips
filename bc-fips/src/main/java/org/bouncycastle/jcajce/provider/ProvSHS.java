...
class ProvSHS
{
    ...
    static class SHAKE128
        extends DigestAlgorithmProvider
    {
        private static final String PREFIX = SHAKE128.class.getName();

        public void configure(final BouncyCastleFipsProvider provider)
        {
            provider.addAlgorithmImplementation("MessageDigest.SHAKE128", PREFIX + "$Digest", new EngineCreator()
            {
                public Object createInstance(Object constructorParameter)
                {
                    return new BaseMessageDigest(FipsSHS.SHAKE128);
                }
            });
            provider.addAlias("MessageDigest", "SHAKE128", NISTObjectIdentifiers.id_shake128);
            provider.addAlias("MessageDigest", "SHAKE128", "SHAKE128-256");
        }
    }

    static class SHAKE256
        extends DigestAlgorithmProvider
    {
        private static final String PREFIX = SHAKE256.class.getName();

        public void configure(final BouncyCastleFipsProvider provider)
        {
            provider.addAlgorithmImplementation("MessageDigest.SHAKE256", PREFIX + "$Digest", new EngineCreator()
            {
                public Object createInstance(Object constructorParameter)
                {
                    return new BaseMessageDigest(FipsSHS.SHAKE256);
                }
            });
            provider.addAlias("MessageDigest", "SHAKE256", NISTObjectIdentifiers.id_shake256);
            provider.addAlias("MessageDigest", "SHAKE256", "SHAKE256-512");
        }
    }
}
