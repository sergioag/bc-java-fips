...
public class JceCMSContentEncryptorBuilder
{
    ...
    private class CMSOutEncryptor
    {
            ...
            throws CMSException
        {
            ...
            KDFCalculator kdf = null;
            try
            {
                kdf = new FipsKDF.AgreementOperatorFactory().createKDFCalculator(
                    FipsKDF.HKDF.withPRF(FipsKDF.AgreementKDFPRF.SHA256).using(encKeyEncoded)
                        .withSalt(hkdfSalt)
                        .withIV(algorithmIdentifier.getEncoded(ASN1Encoding.DER)));
            }
            catch (IOException e)
            {
                throw new CMSException("unable to encode KDF info: " + e.getMessage(), e);
            }
